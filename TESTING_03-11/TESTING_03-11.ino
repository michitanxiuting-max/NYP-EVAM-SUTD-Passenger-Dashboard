...
// Station and track arrays (populated from API)
...
void updateUI_NoLock() 
{
  ...
  if (weather_heavy_rain) 
  {
    ...
    // Set alert header and message for heavy rain
  ...
  else if (weather_heavy_snow || weather_black_ice)
  {
    ...
    // Set alert header and message for snow/ice conditions
  ...
  else 
  {
    ...
    // Clear alert messages when weather is normal
...
void setup() 
{
  ...
  // Pause buttons --> should only appear after they press play
  lv_obj_set_style_bg_opa(ui_pause, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
  lv_obj_set_style_bg_opa(ui_pause2, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
